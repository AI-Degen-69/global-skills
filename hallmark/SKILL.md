---
name: hallmark
description: Use when the user asks to build a new app or landing page, wants to redesign something, invokes Hallmark by na
version: 1.1.0
---
...
> **Hermes port note.** This is a faithful mirror of `Nutlope/hallmark` (upstream `v1.1.0`, MIT). In Hermes there is no `hallmark` CLI binary — map intent to the four verbs below and follow the rule-set directly:
> - **build / design / make a page** → default Design flow.
> - **`hallmark audit <target>`** → read the target, score against `references/slop-test.md` + `references/anti-patterns.md`, return a ranked punch list, **no edits**.
> - **`hallmark redesign <target>`** → keep copy + IA + brand, rebuild the visual structure inside existing implementation boundaries.
> - **`hallmark study <screenshot|URL>`** → extract design DNA (macrostructure, type-pairing, colour anchor); URL mode via `web_extract`, image mode via the vision tool. Refuses pixel-clones / paid templates.
>
> Recommended: wrap an **`audit`** pass after any UI this profile emits (`design` / `frontend-design`) so output auto-refuses the five tells. Regenerate from `https://github.com/Nutlope/hallmark` if upstream advances.
